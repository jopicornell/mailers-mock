...
class Mails extends React.Component {
  ...
  componentDidMount() {
    this._fetchMails();
    // Add keyboard listener once when component mounts
  ...
  _handleKeyboardNavigation(event) {
    ...
    const currentIndex = this.state.mails.findIndex(
      mail => mail === this.state.currentEmail
    );
    ...
    if (isUpKey) {
      ...
      newIndex = Math.min(this.state.mails.length - 1, currentIndex + 1);
    }

    // Only update if index changed
    if (newIndex !== currentIndex) {
      const newEmail = this.state.mails[newIndex];
      this.setState({
        currentEmail: newEmail,
        // Preserve the type if it exists, otherwise use default for new email
        type: this.state.type || (newEmail.template_id ? null : 'text/plain')
      });
    }
  }
